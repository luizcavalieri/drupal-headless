...
export class LazyViewport {
  ...
  private _targets: Map<Element, LazyTarget>;
  public entry: any;
  ...
  private handleIntersectionUpdate = (entries: IntersectionObserverEntry[]): void => {
    ...
    for (this.entry of entries) {
      const lazyTarget = this._targets.get(this.entry.target) as LazyTarget;

      if (lazyTarget) {
        lazyTarget.updateVisibility(
          this.entry.isIntersecting,
          this.entry.intersectionRatio
        );
      }
    }
  }
